...
class Alphabet(ABC):
    @property
    @abstractmethod
    def src_alphabet(self) -> str:
        pass

    @property
    @abstractmethod
    def dst_alphabet(self) -> str:
        pass
    ...
    def alphabet_tsv(self) -> str:
        title = f"#\tUnicode Number\tUnicode x10\tUnicode x16\tPython String"
        lines = [title]
        for n, c in enumerate(self.unknown_chr + self.dst_alphabet):
...
class AlphabetBMP(Alphabet):
    ...
    def get_unigram(self, text: str) -> np.ndarray:
...
def main_alphabet_extract_corpus_alphabet():
    import fargv, glob, tqdm, time, unicodedata
    ...
    def generate_corpus(filenames: Set[str], strip_xml: bool = True, verbose: bool = False) -> Generator[str, None, None]:
        if verbose:
    ...
    total_size = 0
    for file_contnets in generate_corpus(glob_files | corpus_files):
    ...
    if args.output_tsv != "":
        tsv_str = Alphabet(found_alphabet_str).alphabet_tsv
        if args.output_tsv()
